...
export default class TokenAura {
  ...
  destroy() {
    ...
    TokenAura.auras.delete(this.name);
  }
}
...
Hooks.on("canvasTearDown", (canvas) => TokenAura.auras.clear());
...
function makeAura(token) {
  const aura = token.flags[mythacri.id]?.aura ?? {};
  if (!aura.radius || !(aura.radius > 0)) return;
  new TokenAura(token, aura).refresh();
}
...
function refreshAuraOnTokenRefresh(token) {
  const aura = TokenAura.auras.get(token.document.uuid);
...
function deleteAuraOnTokenDelete(token) {
  const aura = TokenAura.auras.get(token.uuid);
...
function createAurasOnTokenUpdate(token, data) {
  const flags = data.flags?.[mythacri.id] ?? {};
