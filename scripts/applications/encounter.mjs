...
export default class Encounter extends HandlebarsApplicationMixin(Application) {
  /** @inheritdoc */
  static DEFAULT_OPTIONS = {
    classes: [mythacri.id, "encounter"],
  ...
  async _prepareContext(options) {
    ...
    const amount = game.settings.get(mythacri.id, "encounter-dice") ?? 1;
   ...
   */
  static #hook(message) {
    const flag = message.flags[mythacri.id]?.encounter === true;
    if (!flag) return;
  ...
  static async #prompt(event, target) {
    ChatMessage.implementation.create({
      content: await renderTemplate("modules/mythacri-scripts/templates/encounter-prompt.hbs", {
        amount: game.settings.get(mythacri.id, "encounter-dice"),
      }),
    });
  }
  ...
  static async #adjust(event, target) {
    ...
    const value = game.settings.get(mythacri.id, "encounter-dice");
    const newValue = Math.max(value + diff, 1);
    await game.settings.set(mythacri.id, "encounter-dice", newValue);
    this.render();
  }
}
